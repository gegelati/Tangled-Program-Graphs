...
class learner
{
   private:
      ...
      double run(vector < instruction * > &, double *);
   ...
   public:
      ...
      string checkpoint();
      inline void clearRegisters(){ memset(_REG, 0, REGISTERS * sizeof(double)); }
      ...
      bool muBid(double, double, double, double, size_t); /* Mutate bid, return true if any changes occured. */
...
struct LearnerBidLexicalCompare {
   bool operator()(learner* l1, learner* l2) { 
      if (l1->key() != l2->key()){ l1->lastCompareFactor(0); l2->lastCompareFactor(0); return l1->key() > l2->key();} //(set to huge val or bid), higher is better
