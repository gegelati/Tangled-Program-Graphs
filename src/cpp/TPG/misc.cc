...
void prepareInFile(ifstream& ifs,const char * dir,const char * prefix, int t=-1, int id=-1, int seed=-1, int phase=-1){
   char inputFilename[80];
   sprintf(inputFilename, "%s/%s.%d.%d.%d.%d.rslt",dir,prefix,t,id,seed,phase);
   ifs.open(inputFilename, ios::in);
   if (!ifs.is_open() || ifs.fail()){
      cerr << "open failed for file: " << inputFilename << " error:" << strerror(errno) << '\n';
      die(__FILE__, __FUNCTION__, __LINE__, "Can't open file.");
   }
}

void prepareOutFile(ofstream& ofs,const char * dir,const char * prefix, int t=-1, int id=-1, int seed=-1, int phase=-1){
   char outputFilename[80];
   sprintf(outputFilename, "%s/%s.%d.%d.%d.%d.%d.rslt",dir,prefix,t,id,seed,phase);
   ...
   if (!ofs.is_open() || ofs.fail()){
      cerr << "open failed for file: " << outputFilename << " error:" << strerror(errno) << '\n';
      die(__FILE__, __FUNCTION__, __LINE__, "Can't open file.");
   }
}
