...
/********************************************************************************************/
string learner::checkpoint(){
	ostringstream oss;
	set < learner * > :: iterator leiter;

	oss << "learner:" << _id << ":" << _gtime << ":" << _action << ":" << _dim << ":" <<  _nrefs;
...
/********************************************************************************************/
learner::learner(long gtime, long action, int maxProgSize, long dim, long id){
...
/********************************************************************************************/
learner::learner(long gtime, learner &plr, long id){
...
/********************************************************************************************/
/* Create learner from checkpoint file */
learner::learner(long gtime, long ancestral_gtime, long action, long dim, long id, long nrefs, vector <instruction *> bid){
		...
		size_t maxProgSize)
{
	...
	if(drand48() < pBidAdd) //if(_bid.size() < maxProgSize && drand48() < pBidAdd)
	{
...
{  
			...
			initer != initerend; initer++, k++)
	{
		...
		if(mode == _mode0) /* Rx <- op Rx Ry */
		{
			srcVal = _REG[((**initer & _srcMask) >> _srcShift).to_ulong() % REGISTERS];
#ifdef SHOWEXEC
...
/********************************************************************************************/
void learner::setFeature(instruction &ins,ulong feat){
	instruction feature( (long) feat );
	//cout << "setFeature m " << _srcMask << endl;
	//cout << "setFeature i " << ins << endl;
	ins = ins & ~_srcMask;
	//cout << "setFeature i " << ins << endl;
	//cout << "setFeature f " << feature << endl;
	feature = feature << _srcShift;
	//cout << "setFeature s " << feature << endl;
	ins = ins | feature;
	//cout << "setFeature d " << ins << endl;
	//cout << endl;
}
